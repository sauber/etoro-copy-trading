...
import { context, ranking, timing } from "📚/trading/testdata.ts";

Deno.test("Instance", () => {
  assertInstanceOf(new Policy(ranking, timing, 0.1), Policy);
});

Deno.test("Purchase Orders", () => {
  const strategy = new Policy(ranking, timing, 0.1);
